...
class ResultStudentExamScheduleViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func setupCustomNavigationTitle() {
        let fullTitle = "Екзамени \(groupName)"
    ...
    private func loadScheduleData() {
        ...
        // URL для екзаменів студентів (без параметрів семестру)
        let baseURL = "https://student.lpnu.ua/students_exam"
        ...
        do {
            ...
            if let noDataMessage = try? viewContent.select(".view-empty").first()?.text(),
               !noDataMessage.isEmpty {
                showAlert(title: "Інформація", message: "Розклад екзаменів не знайдено для групи \(groupName)")
                return
            }
                       
            // Для екзаменів парсимо всі елементи послідовно
            ...
            for element in allElements {
                ...
                if (try? element.hasClass("stud_schedule")) == true {
                    if let lessonContent = try? element.select(".group_content").first() {
                        ...
                        if lines.count > 1 {
                            ...
                            if components.count > 2 {
                                ...
                                let thirdComponent = removeHTMLTags(from: components[2])
                                if thirdComponent.contains(":") {
                                    // Це час екзамену
                                    let timeComponents = thirdComponent.components(separatedBy: "-")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        // Використовуємо звичайний метод, оскільки структура схожа
