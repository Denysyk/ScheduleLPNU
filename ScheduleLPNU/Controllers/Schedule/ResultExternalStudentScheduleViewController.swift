//
//  ResultExternalStudentScheduleViewController.swift
//  ScheduleLPNU
//
...
class ResultExternalStudentScheduleViewController: BaseFullScreenViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func setupNavigationBar() {
        let fullTitle = "Розклад \(groupName) (Заочники)"
    ...
    private func loadScheduleData() {
        ...
        // Формування URL для запиту (без semestrduration для заочників)
        let baseURL = "https://student.lpnu.ua/parttime_schedule"
    ...
    @objc private func refreshSchedule(_ sender: Any? = nil) {
        ...
        if !isInternetAvailable() {
            showAlert(title: "Немає інтернету", message: "Неможливо оновити розкладфжез інтернет-з'єднання")
    ...
    @objc private func saveSchedule() {
        ...
        let schedule = SavedSchedule(
            id: "external_\(groupName)_\(semester)",
            title: "Розклад \(groupName) (Заочники)",
            type: .external,
            groupName: groupName,
            teacherName: nil,
            semester: semester,
                ...
                for dayHeader in dayHeaders {
                    if let dayName = try? dayHeader.text() {
                        ...
                        while nextElement != nil {
                            ...
                            if let studScheduleDiv = try? nextElement?.select(".stud_schedule").first() {
                                if let lessonRows = try? studScheduleDiv.select(".views-row") {
                                    for lessonRow in lessonRows {
                                        ...
                                        if let lessonContent = try? lessonRow.select(".group_content").first() {
                                            ...
                                            if lines.count > 1 {
                                                ...
                                                // Аудиторія залишається порожньою для заочників
                                                room = ""
                                            }
                                            ...
                                            // Залишаємо їх порожніми, як на оригінальному сайті
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        // Для заочників показуємо дати як є
