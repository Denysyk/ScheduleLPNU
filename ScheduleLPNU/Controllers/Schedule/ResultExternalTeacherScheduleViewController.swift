//
//  ResultExternalTeacherScheduleViewController.swift
//  ScheduleLPNU
//
...
class ResultExternalTeacherScheduleViewController: BaseFullScreenViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    private func setupNavigationBar() {
        let fullTitle = "\(teacherName) (заочне)"
    ...
    private func loadScheduleData() {
        ...
        let baseURL = "https://staff.lpnu.ua/lecturer_schedule_parttime"
        let encodedTeacher = teacherName.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) ?? teacherName
        let urlString = "\(baseURL)?teachername=\(encodedTeacher)&semestr=\(semesterValue)"
    ...
    @objc private func saveSchedule() {
        ...
        let schedule = SavedSchedule(
            id: "externalTeacher_\(teacherName)_\(semester)",
            title: "\(teacherName) (заочне)",
    ...
    private func parseScheduleFromHTML(_ html: String) {
        do {
            ...
            // Парсимо всі елементи послідовно (схоже до paste.txt)
                        ...
                        for lessonRow in lessonRows {
                            ...
                            if let lessonContent = try? lessonRow.select(".group_content").first() {
                                ...
                                if lines.count > 1 {
                                    ...
                                    // ВИПРАВЛЕНИЙ парсинг для заочників:
                                    // Формат: "ЕВ-31з, практична"
                                    // Тобто: група, тип заняття (без аудиторії)
                                    let components = details.components(separatedBy: ", ")
                                        .map { $0.trimmingCharacters(in: .whitespacesAndNewlines) }
                                        .filter { !$0.isEmpty }
                                    
                                    if components.count >= 2 {
                                        // Перший компонент - група
                                        groupName = removeHTMLTags(from: components[0])
                                        // Другий компонент - тип заняття
                                        type = removeHTMLTags(from: components[1])
                                        // Аудиторії немає для заочників
                                        room = ""
                                    } else if components.count == 1 {
                                        // Якщо тільки один компонент, це може бути група
                                ...
                                // НЕ заповнюємо порожні поля - залишаємо як є
                                if lessonName.isEmpty { lessonName = "Невідомо" }
                                
                                let lesson = Lesson(
                                    number: currentLessonNumber,
                                    name: lessonName,
                                    teacher: groupName, // Для викладачів це група
                                    room: room, // Порожнє для заочників
                                    type: type,
    ...
    func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
        ...
        // Для заочників відображаємо дату як є (2025-01-25)
